...
try:
    ...
    #query = f"SELECT * FROM {redshift_schema}.{redshift_table};"  # Usar el esquema y tabla
    ...
    # Pasamos la comumna fecha a formato datatime
    ...
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=10))  # Mostrar cada 2 días
    ...
    ultimos_7_dias = df[df['fecha'] >= (df['fecha'].max() - pd.Timedelta(days=30))]
    ...
    # Graficar las columnas seleccionadas
    ax.plot(ultimos_7_dias['fecha'], ultimos_7_dias['precio_apertura'], label='Precio de Apertura', marker='o')
    ax.plot(ultimos_7_dias['fecha'], ultimos_7_dias['precio_cierre'], label='Precio de Cierre', marker='o')
    ax.plot(ultimos_7_dias['fecha'], ultimos_7_dias['vw_aprice'], label='VWAP', marker='o') 
    ...
    st.error(f"Ocurrió un error al conectar a Redshift: {e}")
finally:
    if conn:
